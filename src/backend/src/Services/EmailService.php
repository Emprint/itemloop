...
class EmailService
{
    ...
    public function __construct()
    {
        ...
        $this->appName    = AppSettingsController::get('app_name', $_ENV['MAIL_FROM_NAME'] ?? 'Itemloop');
        $this->fromName   = $this->appName;
    }

    /**
     * Return all admin users who have opted in to receive admin notification emails.
     * Returns array of ['email' => ..., 'name' => ..., 'locale' => ...]
     */
    public static function getAdminRecipients(): array
    {
        try {
            $db   = \App\Database::get();
            $stmt = $db->query(
                "SELECT name, email, locale FROM users
                 WHERE role = 'admin' AND notify_admin_emails = 1 AND status = 'active'"
            );
            return $stmt->fetchAll(\PDO::FETCH_ASSOC);
        } catch (\Throwable $e) {
            error_log('EmailService::getAdminRecipients failed — ' . $e->getMessage());
            return [];
        }
    }
    ...
    public function sendTemplate(
        ...
        // Inject app name so templates don't hardcode it
        ...
        $subjects = [
            'en' => [
                ...
                'pending-user-notification'    => 'New user awaiting approval',
            ],
            'fr' => [
                ...
                'pending-user-notification'    => 'Nouvel utilisateur en attente d\'approbation',
            ],
        ];

        $lang    = isset($subjects[$locale]) ? $locale : 'en';
        $subject = $subjects[$lang][$template] ?? ($this->appName . ' notification');
