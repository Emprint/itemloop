...
| ID  | Role | User Story | Priority | Complexity | Status | Notes |
|-----|------|-----------|----------|------------|--------|-------|
...
| US6 | User | As a user, I want to scan a barcode to quickly search or add an item. | Medium | 🔴 Complex | ✅ | Barcode search includes barcode field; single match opens product in read-only view |
...
| US9 | User | As a user, I want to use the app from a smartphone or browser so I can manage inventory anywhere. | High | 🟡 Medium | ✅ | PWA; iOS Safari fixes: sidebar uses `100svh` so the Login link is not hidden by the bottom address bar; `env(safe-area-inset-bottom)` padding for home indicator; scroll resets to top when opening a product detail; browser scroll restoration disabled (`history.scrollRestoration = 'manual'`) |
...
| US20 | User | As a user, I want the app interface to automatically detect my system language and allow me to manually switch between English and French. | High | 🟡 Medium | ✅ | Language switcher is styled toggle buttons (EN · FR) in the sidebar — no native dropdown; buttons are dynamically rendered from a `supportedLangs` array with flex-wrap for scalability; adding a new language only requires adding one entry to the array |
...
| US42 | Admin | As an admin, I want to export inventory or order data as a formatted PDF report or an Excel spreadsheet (.xlsx), so I can share or archive summaries and perform further analysis in spreadsheet software. | Low | 🟡 Medium | ⚪ | |
...
| US45 | Editor/Admin | As an editor or admin, I want a dedicated page to manage product attribute values (categories, conditions, colors) — listing all values with usage counts, renaming them inline, and deleting them with the option to reassign products to a replacement value before deletion — so the taxonomy stays clean and consistent over time. | Medium | 🟡 Medium | ✅ | `/admin/attributes` page; tabs for categories, conditions, colors; inline rename; delete with usage count check and reassignment picker; accessible from Settings dashboard |
| US46 | User | As a user, I want a profile page (accessible by clicking my avatar in the sidebar) where I can update my display name, change my password, and set my preferred language, so I can personalize my account. | Medium | 🟡 Medium | ✅ | `/profile` page with name, locale picker, change-password form; avatar and name in navbar link to profile; also accessible from Settings dashboard |
| US47 | System | As the system, I want to store each user's preferred locale (e.g. `en` or `fr`) — defaulting to the browser language at registration — and use it when sending emails to that user, so emails arrive in the user's own language regardless of the app's global language setting. | Medium | 🟢 Simple | ✅ | `users.locale` column; detected from `Accept-Language` on registration; editable in profile; all outgoing emails use user's locale |
| US48 | User | As a user, I want a "Forgot password?" link on the login page that lets me receive a password-reset email and set a new password via a secure time-limited link. | Medium | 🟡 Medium | ✅ | `/auth/forgot-password` and `/auth/reset-password` pages; `password_reset_tokens` table; token expires after 1 hour; bilingual reset email; new-password validation enforced on the frontend (same rules as registration: 8+ chars, letter, digit, special character) |
| US49 | Admin | As an admin, I want to configure the application name in settings, so that the navbar header and all outgoing emails display the organisation's own name instead of "Itemloop". | Low | 🟢 Simple | ✅ | `app_name` key in `app_settings` table (max 50 chars, defaults to "Itemloop"); editable on the General Settings page; injected into all email templates; displayed in navbar |
| US50 | Admin | As an admin, I want to (1) opt in or out of receiving admin notification emails per-account, and (2) have all opted-in admins notified in their own locale when a new order is placed or a user registers with pending status — so the right people are informed without requiring a hardcoded email address in configuration. | Medium | 🟡 Medium | ✅ | `notify_admin_emails` flag (default 1 for existing admins, 0 for new ones) on each user; editable in profile (admin users) and user management form (admin role rows); `EmailService::getAdminRecipients()` queries opted-in active admins; order and pending-registration emails loop over recipients sending each in their own locale; pending-user-notification bilingual email template |
| US51 | Admin | As an admin, I want to view a log of outgoing emails (recipient, subject, status, timestamp) in the Settings area, so I can verify that notifications are being delivered correctly without exposing message content. | Low | 🟢 Simple | ✅ | `/admin/settings/email-logs` page; shows sent/failed status, recipient address, subject, and timestamp; read-only, no message body shown; accessible from Settings dashboard |
| US52 | User | As a user browsing on a mobile device, I want product images to appear as a full-width swipeable carousel at the top of the product detail view (read-only mode), so I can quickly view all photos without scrolling past details first. Images are shown uncropped (contain fit) with left/right arrows, dot indicators, and swipe gesture support. | Medium | 🟡 Medium | ✅ | Carousel rendered above form fields on ≤1024px viewports, read-only mode only; hidden on desktop (thumbnail grid still used there); swipe handled via touchstart/touchend; carousel index resets when switching products |
